                ...
                {items.map((item, i) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      "w-full justify-start group flex items-center gap-x-2.5 rounded-md px-2 py-1.5 text-sm font-medium leading-6 text-zinc-700 dark:text-zinc-400 hover:bg-gray-50 transition"
                    )}
                    onClick={onClose}
                  >
...
const Layout = ({ children }: PropsWithChildren) => {
  ...
  return (
    <div className="relative h-screen flex flex-col md:flex-row overflow-hidden">
      {/* sidebar for desktop */}
      <div className="hidden md:block w-64 lg:w-80 border-r p-6 h-full text-brand-900 relative z-10">
        <Sidebar />
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="md:hidden flex items-center justify-between p-4 border-b">
          <p className="text-lg/7 font-semibold text-brand-900">Quala</p>
          <button
